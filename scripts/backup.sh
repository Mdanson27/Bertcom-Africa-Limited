...
# Generated dynamically by Copier for Bertcom Africa Business Os
#
# Usage (directly):
...
# Via make:
#   make db-backup
#   make db-restore FILE=backups/postgres/bertcom_db_20260820_120000.dump
#
# Features
# --------
...
POSTGRES_USER="${POSTGRES_USER:-bertcom_user}"
POSTGRES_PASSWORD="${POSTGRES_PASSWORD:-Zimbarent@2026}"
POSTGRES_DB="${POSTGRES_DB:-bertcom_db}"
...
ENV_FILE="${ENV_FILE:---env-file .env}"

if command -v "${CONTAINER_ENGINE}-compose" &>/dev/null; then
    COMPOSE_CMD=("${CONTAINER_ENGINE}-compose" ${ENV_FILE} -f "${COMPOSE_FILE}")
else
    COMPOSE_CMD=("${CONTAINER_ENGINE}" compose ${ENV_FILE} -f "${COMPOSE_FILE}")
fi
